...
export class Topology {
  static difference = (origin: GeolocusGeometry, target: GeolocusGeometry) => {
    ...
    const result = jsts.operation.overlay.OverlayOp.difference(originGeometry, targetGeometry)
  ...
  static intersection = (polygon0: GeolocusGeometry, polygon1: GeolocusGeometry) => {
    const geometry0 = polygon0.getGeometry()
    const geometry1 = polygon1.getGeometry()
    const intersection = jsts.operation.overlay.OverlayOp.intersection(geometry0, geometry1)
  ...
  private static buffer = (geometry: GeolocusGeometry, distance: EuclideanDistance) => {
    const buffer = jsts.operation.buffer.BufferOp.bufferOp(geometry.getGeometry(), distance)
    return buffer
  }

  static bufferOfDistance = (geometry: GeolocusGeometry, distance: EuclideanDistance) => {
  ...
  static bufferOfRange = (geometry: GeolocusGeometry, range: EuclideanDistanceRange) => {
    const min = Math.min(...range)
    const max = Math.max(...range)

    const buffer0 = this.buffer(geometry, min)
    const buffer1 = this.buffer(geometry, max)
    const difference = jsts.operation.overlay.OverlayOp.difference(buffer1, buffer0)
